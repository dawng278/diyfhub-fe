...
const MoviesGridByCountry = () => {
  ...
  console.log('🌍 MoviesGridByCountry Component Rendered!');
  ...
  // Extract country name from slug (e.g., "trung-quoc" -> "Trung Quốc")
  const formatCountryName = (slug) => {
    if (!slug) return 'Quốc gia';
    
    // Map common country slugs to proper Vietnamese names
    const countryMap = {
  ...
  const fetchMovies = useCallback(async (page = 1) => {
    try {
      ...
      if (!countrySlug) {
        console.error('No countrySlug provided!');
        navigate('/');
        return;
      }

      console.log(`Fetching movies for country slug: ${countrySlug}, page: ${page}`);
      
      const response = await getMoviesByCountry(countrySlug, {
        page,
      ...
      if (!moviesData || !Array.isArray(moviesData) || moviesData.length === 0) {
        setError('Không tìm thấy phim nào trong quốc gia này');
        setMovies([]);
        setTotalPages(1);
        setTotalItems(0);
        setCountryName(formatCountryName(countrySlug));
        return;
      }
      ...
      // Set country name from the first movie's country data if available
      if (processedMovies.length > 0 && processedMovies[0].country && Array.isArray(processedMovies[0].country)) {
        const countryObj = processedMovies[0].country.find(c => c.slug === countrySlug);
        if (countryObj) {
  ...
  useEffect(() => {
    console.log('=== 🔄 MoviesGridByCountry useEffect TRIGGERED ===');
    console.log('countrySlug:', countrySlug);
    ...
    if (countrySlug) {
      ...
      console.warn('❌ No countrySlug found!');
    }
  }, [currentPage, fetchMovies, countrySlug]);
      ...
      <div className="min-h-screen bg-gray-50 py-6">
        <div className="container mx-auto px-3">
          <h1 className="text-2xl font-bold text-gray-800 mb-6">
            Đang tải phim {countryName || formatCountryName(countrySlug)}...
          </h1>
      ...
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="container mx-auto px-3">
          <div className="max-w-2xl mx-auto bg-white rounded-lg shadow p-8 text-center">
            ...
            <p className="text-gray-600 mb-4">Không có phim nào trong quốc gia này.</p>
            <button
              onClick={() => navigate('/')}
              className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md transition-colors"
            >
    ...
    <div className="min-h-screen bg-gray-50 py-6">
      <div className="container mx-auto px-3">
        <h1 className="text-2xl font-bold mb-6 text-gray-800">
          Phim {countryName || formatCountryName(countrySlug)}
...
export default MoviesGridByCountry;